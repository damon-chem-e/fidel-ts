...
def patch_reconstruction_vectorized(
    ...
    # If we use total_length directly, it might imply a different number of patches than N
    covered_length = (N - 1) * stride + patch_len
    ...
    # Ensure output matches pred_len
    if covered_length < pred_len:
        ...
        padding = torch.zeros(B, C, pred_len - covered_length, device=x.device)
