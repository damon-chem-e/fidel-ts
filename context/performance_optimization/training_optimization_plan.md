...
**Root Cause (CONFIRMED)**: The dataloader is CPU-bound, unable to feed the GPU fast enough. The GPU starves waiting for data. Profiling confirms **18.6 ms per sample overhead** in `__getitem__()`.
...
## Profiling Results (Actual Data)

**Profiling artifacts**:
- JSON results: [`profiling_germany_renewable.json`](./profiling_germany_renewable.json)
- Full session log: [`profiling_session_germany.log`](./profiling_session_germany.log)

**Dataset**: Germany_Renewable_Power_Grid (lynx_film model)
- Input length: 360, Output length: 168
- Batch size: 768

### Per-Sample Overhead: 18,570 μs (18.6 ms)

| Operation | % of Time | Mean (μs/call) | Calls/Sample | Notes |
|-----------|-----------|----------------|--------------|-------|
| **hetero_downtime_check** | **34.2%** | **3,171** | 2 | **#1 BOTTLENECK** - Non-vectorized Python loop |
| hetero_time_matching | 5.8% | 536 | 2 | pd.searchsorted - expected |
| hetero_embedding_lookup | 4.6% | 423 | 2 | Dict lookup + shape normalization |
| hetero_array_construction | 3.4% | 319 | 2 | np.array + np.concatenate |
| ts_array_slicing | 0.0% | 2.7 | 1 | Negligible |
| sample_id_generation | 0.0% | 2.7 | 1 | Negligible |

**Key Finding**: `hetero_downtime_check` is the #1 bottleneck (34.2% of time), not `hetero_time_matching` as originally hypothesized. This is due to a non-vectorized Python list comprehension in `data_loader.py:816`:
```python
...
- First batch: ~97 seconds (cold start, embedding loading)
- Subsequent batches: **~7 seconds each** (768 samples)
- GPU transfer: ~35 ms per batch (negligible - 0.5%)
- Throughput: **~110 samples/second** (extremely slow)

### Impact on Training

With 200,000 training batches per epoch and ~7s per batch:
- **Estimated epoch time: ~389 hours** (without accounting for forward/backward pass)
- This explains why training takes >1 day even for small experiments
